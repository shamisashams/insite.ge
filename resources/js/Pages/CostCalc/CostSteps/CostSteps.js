...
import {CostBtn, CostOption, CostQues, CostTitle, CommentField} from "./CostBox";
import {Link} from '@inertiajs/inertia-react'
import {Inertia} from '@inertiajs/inertia'

import "./CostSteps.css";

const CostSteps = ({step, click, back, features, parentCallback, state, price}) => {
    ...
    const handler = (e, startPrice) => {
        ...
        if (e.target.checked) {
            ...
            let index = checkboxItems[featureId]['answers'].findIndex(item => item.id === answerId)
    ...
    useEffect(() => {
        ...
        textareaRef.current = features.map(
            (item, index) => {
    ...
    const toggleDisabled = (e) => {
        ...
        if (checkboxItems[featureId]) {
            checkboxItems[featureId]['comment'] = textareaRef.current[featureId].value;
        } else {
            checkboxItems[featureId] = {
                'answers': [],
    ...
    let style = window.innerWidth <= 800 ? {
        top: '100px',
        left: step === 1 ? "-2vw" : '-' + ((step * 100) - 98) + 'vw',
        justifyContent: "unset"
    } : (window.innerWidth > 800 ? {top: step === 1 ? "-7vh" : '-' + ((step * 100) - 100) + "vh"} : {})
                ...
                features.map((feature, index) => (
                    <div className="each_step" key={index}>
                        ...
                        <div className="cost_box">
                            ...
                            <form>
                            {
                                feature.answer.map((item, index) => (
                                    <CostOption key={index} featureId={feature.id}
                                                ...
                                                handler={(e) => handler(e, item.start_price)} id={item.id}
                                                option={item.title}/>
                                ))
                            }
                            </form>
                            ...
                            <CommentField
                                cmFieldClass={
                                    commentSection[feature.id] ? "comment_field" : "comment_field hidden"
                                }
